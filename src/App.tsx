  ...
  useEffect(() => {
    const run = async () => {
      if (!("LanguageDetector" in self)) {
        if (notSupportedRef.current) notSupportedRef.current.hidden = false;
        return;
      }
      ...
      inputRef.current.addEventListener("input", async () => {
        ...
        if (!value) {
          detectedRef.current!.textContent = "not sure what language this is";
          return;
        }

        const [{ detectedLanguage, confidence }] = await detector.detect(value);
        detectedRef.current!.textContent = `${(confidence * 100).toFixed(
          1
      ...
      if ("Translator" in self) {
        formRef.current.addEventListener("submit", async (e) => {
          ...
          try {
            ...
            outputRef.current!.textContent = await translator.translate(
  ...
  return (
    <>
      ...
      <form ref={formRef}>
        ...
        ></textarea>
        <p>
          I'm <span ref={detectedRef}>not sure what language this is</span>.
        </p>
